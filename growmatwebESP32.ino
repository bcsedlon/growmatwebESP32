...
void drawFrame1(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  display->setTextAlignment(TEXT_ALIGN_RIGHT);
  display->drawString(X2 + x, 20 + y, myIP.toString());
*/
  display->setFont(ArialMT_Plain_16);
  display->setTextAlignment(TEXT_ALIGN_LEFT);
  if(!ap) { //myIP == IPAddress(0,0,0,0)) {
	  display->drawString(0 + x, 16 + y, WiFi.localIP().toString());
  }
  else {
	  display->drawString(0 + x, 16 + y, myIP.toString());
  }
...
void drawFrame3(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  //display->drawString(0 + x, 10 + y, "IP: " + myIP.toString());
  display->setTextAlignment(TEXT_ALIGN_LEFT);
  ...
  if(digitalRead(CONFIG_WIFIAP_PIN) == LOW) {
	  ...
	  if ( digitalRead(CONFIG_WIFI_PIN) == LOW ) {
		  ...
		  Serial.println("Starting AP for reconfiguration ...");
  		  ...
  		  if(!wifiManager.autoConnect("GROWMATWEB", "GROWMATWEB")) {
